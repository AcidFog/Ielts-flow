...
const UserInputDialog = ({children,coachingOption}) => {
    ...
    const createDisscussionRoom=useMutation(api.DiscussionRoom.CreateNewRoom);
    ...
    const OnClickNext= async()=>{
        setLoading(true);
        const result=await createDisscussionRoom({
            topic:topic,
    ...
    <DialogHeader>
      ...
      <DialogDescription asChild>
        <div className='mt-3'>
            ...
            <h2 className='text-black'>Select your Expert</h2>
            <div className='grid grid-cols-3 md:grid-cols-5 gap-6 mt-3'>
                {CoachingExpert.map((expert,index)=>(
                    ...
                    width={100}
                    height={100}
                    className={`rounded-2xl h-[80px] w-[80px] object-cover hover:scale-105 transition-all cursor-pointer p-1
